...
Again, please don't attempt to use this if you'll be searching giant databases, 
as I'd imagine you would need something performance aware, 
which this is _not_. But. It should work fine for hundreds, thousands, 
maybe even tens-of-thousands of records. You know, like blogs, small forums, etc.

h3. Configuration

In config/initializers/easy_search_setup.rb (没有就自己建一个该文件)(or config/environment.rb if you aren't running Rails 2.0+),
...
We'll go straight into it. First thing, you install the plugin. 
Then you add a generic class and give it the @EasySearch@ functionality. Like so:
...
Now we have a @Search@ class that can easily handle search your @ActiveRecord@ models! 
Notice how this class _does not_ descend from @ActiveRecord@.

h3. Usage

In words, let's say we want to "search the users table with 'ryan heath'". 
To represent that in code, it's basically the same thing:
